#!/usr/bin/env python3
"""Run `cargo test` for all targets listed in a TOML file across shared config TOMLs."""
...
from test_matrix import list_config_files, run_test_matrix
...
def parse_targets_file(targets_path: Path) -> tuple[list[TargetSpec], dict[str, Any]]:
    ...
    for i, row in enumerate(rows, start=1):
        ...
        if not isinstance(doc, bool):
            raise ValueError(f"target '{crate}' has invalid 'doc': expected bool")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Delete existing output and re-run tests",
    )
    parser.add_argument(
        "--fail-fast",
        action="store_true",
...
def main() -> int:
    ...
    if built_count == 0:
        print("No enabled targets found to test.")
