...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if fs::exists("../libafl_libfuzzer_runtime")? && !cfg!(feature = "libafl-libfuzzer-use-version")
    {
        ...
        #[cfg(unix)]
        {
            ...
            // Canonicalize libafl_leaf path
            {
                ...
                let Some(&mut toml::Value::String(ref mut libafl_leaf_path)) = Some(root)
                    .and_then(|r| r.entry("patch").or_insert_with(empty_table).as_table_mut())
                    .and_then(|p| {
                        p.entry("crates-io")
    ...
    // <<<<<<< Leaf's modification
    /* The native library generated by this package contains important symbols.
     * Let's make sure `leafc` does not modify it at all.
     * Anyway, they library is not meant to be called by the instrumented version of the program. */
    command.env("LEAFC_FORCE_NOOP", "1");
    // >>>>>>> Leaf's modification
