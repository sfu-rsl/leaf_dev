...
use common::{directed::RawCaseValue, pri::BasicBlockLocation};

use crate::{SwitchTrace, TraceSwitchStep, trace::TraceConstraint};
...
pub(crate) async fn read_switch_trace(
    ...
    // jsonl_sync::JsonListTraceReader {
    bincode_sync::BinaryStreamTraceReader {
...
mod jsonl_sync {
    ...
    use common::types::trace::BranchRecord;
    ...
    pub(super) fn read_jsonl_sym_trace<R>(
        reader: &mut R,
    ) -> Vec<(usize, (BasicBlockLocation, TraceConstraint))>
    where
        R: Read,
    {
        let records = serde_json::Deserializer::from_reader(reader).into_iter::<Value>();
    ...
    pub(super) fn read_jsonl_precondition_trace<R>(
        reader: &mut R,
    ) -> Vec<(usize, (BasicBlockLocation, Vec<usize>))>
    where
        R: Read,
    {
        let records = serde_json::Deserializer::from_reader(reader).into_iter::<Value>();
    ...
    impl<R: Read> TraceReader for JsonListTraceReader<R> {
        ...
        fn read_sym_trace(&mut self) -> Vec<(usize, (BasicBlockLocation, TraceConstraint))> {
            read_jsonl_sym_trace(&mut self.sym_trace_reader)
        }
        ...
        fn read_precondition_trace(&mut self) -> Vec<(usize, (BasicBlockLocation, Vec<usize>))> {
            ...
            read_jsonl_precondition_trace(precondition_reader)
        }
    }
}
...
mod bincode_sync {
    ...
    pub(super) struct BinaryStreamTraceReader<R> {
        pub full_trace_reader: R,
        pub sym_trace_reader: R,
        pub preconditions_reader: Option<R>,
    }

    impl<R: Read> TraceReader for BinaryStreamTraceReader<R> {
        fn read_full_trace(
            &mut self,
        ) -> Vec<(usize, (BasicBlockLocation, ConstraintKind<RawCaseValue>))>
        where
            R: Read,
        {
            let reader = &mut self.full_trace_reader;
            let mut reader = BufReader::new(reader);

            #[derive(bincode::Decode)]
            struct Record {
                value: ExeTraceRecord<RawCaseValue>,
                index: usize,
            }
            let mut result = Vec::<Record>::new();

            while reader.has_data_left().expect("Error in reading full trace") {
                result.push(
                    bincode::decode_from_std_read(
                        &mut reader,
                        bincode::config::standard().with_variable_int_encoding(),
                    )
                    .expect("Error in decoding full trace record"),
                );
            }

            result
                .into_iter()
                .filter_map(|Record { value, index }| match value {
                    ExeTraceRecord::Branch(BranchRecord {
                        location,
                        decision: kind,
                    }) => Some((index, (location, kind))),
        ...
        fn read_sym_trace(&mut self) -> Vec<(usize, (BasicBlockLocation, TraceConstraint))> {
            // FIXME: Use json lines until we have a shared fixed interface
            jsonl_sync::read_jsonl_sym_trace(&mut self.sym_trace_reader)
        }
        ...
        fn read_precondition_trace(&mut self) -> Vec<(usize, (BasicBlockLocation, Vec<usize>))> {
            ...
            jsonl_sync::read_jsonl_precondition_trace(precondition_reader)
        }
    }
}
