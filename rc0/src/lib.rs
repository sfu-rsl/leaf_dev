...
use rustc_middle::{
    ...
    mir::{
        ...
        BasicBlock, BasicBlockData, Body, LocalDecl, Operand, Place, Rvalue, SourceInfo,
        StatementKind,
    },
    ty::{query, TyCtxt},
};
use rustc_span::def_id::DefId;
use std::{collections::HashMap, ops::Index, ops::IndexMut, path::PathBuf};
...
fn local_optimized_mir<'tcx>(
    ...
    Transformer::new(tcx, body).transform();
...
impl<'tcx> Transformer<'tcx> {
    fn new(tcx: TyCtxt<'tcx>, body: &'tcx Body<'tcx>) -> Transformer<'tcx> {
        let body = tcx.alloc_steal_mir((*body).clone()).steal();
    ...
    fn transform(&mut self) {
        ...
        let mut reverse = self.clear_and_get_reverse();

        // Create a new block for each statement
        let index_counts = self.repopulate_basic_blocks(&mut reverse);

        // Calculate index mappings
        let index_map = self.create_index_mappings(&index_counts);

        //let basic_blocks = self.body.basic_blocks_mut();
        ...
        //            .flatten();
        //        self.process_terminator(basic_block_idx, basic_block, &index_map, next_terminator);
        //    }
        //}
        ...
        self.body
            .basic_blocks()
            .indices()
            .for_each(|basic_block_idx| {
                //let next_terminator = iter
                //    .peek()
                //    .map(|(_, next_block)| next_block.terminator.as_ref())
                //    .flatten();
                //self.process_terminator(basic_block_idx, &index_map, next_terminator);
                self.process_terminator(basic_block_idx, &index_map);
            });
    }

    fn clear_and_get_reverse(&mut self) -> Vec<BasicBlockData<'tcx>> {
        let basic_blocks = self.body.basic_blocks_mut();
        let mut reverse = vec![];
    ...
    fn repopulate_basic_blocks(&mut self, reverse: &mut Vec<BasicBlockData<'tcx>>) -> Vec<usize> {
        let mut index_counts = vec![];
        while let Some(basic_block) = reverse.pop() {
            self.create_new_blocks(basic_block, &mut index_counts);
        }
    ...
    fn create_index_mappings(&self, index_counts: &Vec<usize>) -> HashMap<BasicBlock, BasicBlock> {
    ...
    fn process_terminator(
        ...
        let basic_block = self.body.index(basic_block_idx);
        match &basic_block.terminator {
            Some(_) => {
                // The basic block already has a terminator (it's the last basic block).
                // Adjust jump targets.
                self.remap_jump_targets(basic_block_idx, &index_map);
            }
            None => {
                // A newly-added basic block
                self.add_new_terminator(basic_block_idx);
            }
        };
    }
    ...
    fn remap_jump_targets(
        ...
        // Adjust jump targets
        let terminator = &mut self.body.index_mut(basic_block_idx).terminator_mut();
    ...
    fn add_new_terminator(&mut self, basic_block_idx: BasicBlock) {
        let is_empty = self.body.index(basic_block_idx).statements.is_empty();
        let new_terminator = Some(if is_empty {
            let next_terminator = self
                .body
                .basic_blocks()
            ...
            match next_terminator.unwrap().kind {
                SwitchInt {
                    ...
                    switch_ty: _,
                    targets: _,
                } => self.get_switch_int(basic_block_idx),
                Return => self.get_ret(basic_block_idx),
                Call {
                    func: _,
                    args: _,
                    destination: _,
            ...
            let kind = &self
                ...
                .clone();
            match kind {
                StatementKind::Assign(asgn) => self.get_assign(basic_block_idx, asgn),
                // TODO: Check if we need to handle anything else.
                _ => self.get_goto(basic_block_idx),
            }
        });
        let basic_block = self.body.index_mut(basic_block_idx);
        basic_block.terminator = new_terminator;
    }

    // TODO: Filler
    fn get_switch_int(&mut self, basic_block_idx: BasicBlock) -> Terminator<'tcx> {
        ...
        let ret_local_decl_idx = self.body.local_decls.push(ret_local_decl);
    ...
    // TODO: Filler
    fn get_ret(&mut self, basic_block_idx: BasicBlock) -> Terminator<'tcx> {
    ...
    // TODO: Filler
    fn get_call(&mut self, basic_block_idx: BasicBlock) -> Terminator<'tcx> {
    ...
    fn get_assign(
        ...
        match rvalue {
            ...
            */
            Rvalue::Use(operand) => self.get_assign_use(place, basic_block_idx, operand),
            _ => {
    ...
    fn get_goto(&self, basic_block_idx: BasicBlock) -> Terminator<'tcx> {
        Terminator {
            source_info: SourceInfo::outermost(rustc_span::DUMMY_SP), // TODO: Not sure how good
            kind: Goto {
...
fn get_assign<'tcx>(
    ...
    match rvalue {
        ...
        */
        Rvalue::Use(operand) => get_assign_use(tcx, func_map, place, basic_block_idx, operand),
        _ => {
